//
//  LifestyleGraphViewController.m
//  MyPsych
//
...
@interface LifestyleGraphViewController ()
...
@implementation LifestyleGraphViewController
...
- (NSUInteger)numberOfRecordsForPlot:(CPTPlot *)plot {
    ...
    for (int i = 0; i < LIFESTYLE_MINIMUM_COUNT; i++) {
        if ([[plot title] caseInsensitiveCompare:kLifestyleEntryIdentifiers[i]] == NSOrderedSame) {
            index = i;
        }
    }
    BOOL isHidden = [[ProfileDataManager sharedManager] lifestyleGraphHiddenAtIndex:index];
    if (isHidden)
        return 0;
    
    return [[ProfileDataManager sharedManager] lifestyleEntryCount];
}
...
- (NSNumber*)numberForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index {
	LifestyleDataEntry* entry = [[ProfileDataManager sharedManager] lifestyleEntryForIndex:index];
    NSNumber* num = nil;
	if (fieldEnum == CPTScatterPlotFieldX) {
        ...
        for (int i = 0; i < LIFESTYLE_MINIMUM_COUNT; i++) {
            ...
            if ([idStr caseInsensitiveCompare:kLifestyleEntryIdentifiers[i]] == NSOrderedSame) {
...
- (void)initializeGraph {
    ...
    // self.thisGraph.title = @"Lifestyle Graph";
    ...
    for (int index = 0; index < LIFESTYLE_MINIMUM_COUNT; index++) {
        CPTScatterPlot* scatterPlot = [[CPTScatterPlot alloc] init];
        [plotsArray addObject:scatterPlot];
        [scatterPlot setDelegate:self];
        [scatterPlot setDataSource:self];
        [scatterPlot setIdentifier:kLifestyleEntryIdentifiers[index]];
        [scatterPlot setTitle:kLifestyleEntryIdentifiers[index]];
